...
import { find, flatten, get, isFunction, omit } from 'lodash'
import moment from 'moment'
import React from 'react'
import { useDrag, useDrop } from 'react-dnd'
import { SortableContainer, SortableElement } from 'react-sortable-hoc'
import BuiltInComponents from '../Form/components'
import { getClassNames, getComponentName } from './config'
...
const EditableDraggableRow = props => {
  ...
  if (rowDraggable) {
    ...
    drop(drag(dragRef))
    return getTabeTrNode(<tr ref={dragRef} className={`${className} ${isOver ? dropClassName : ''}`} {...resetProps} style={{ ...style, cursor: 'move' }} />)
  }
    ...
    if (editable) {
      ...
      if (Object.keys(BuiltInComponents).includes(tpl)) {
        ...
        if (tpl === 'input') {
          ...
          FormItemNodeProps.onMouseLeave = save
