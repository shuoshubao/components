...
import { EllipsisOutlined, MenuOutlined, UserOutlined } from '@ant-design/icons'
import { classNames, formatTime, getLabelByValue, isEmptyArray, isEmptyObject, isEmptyValue, multiply, stringifyUrl, thousands } from '@nbfe/tools'
import { Avatar, Button, Dropdown, Image, Modal, Popconfirm, Progress, Rate, Tag, Tooltip, Typography } from 'antd'
import { filter, find, flatten, get, isFunction, isObject, isString, merge, noop, omit } from 'lodash'
import { SortableHandle } from 'react-sortable-hoc'
import BuiltInComponents from '../Form/components'
import { getClassNames, getOnceImageFallback, getTooltipTitleNode } from './config'
...
export const renderButton = (item, context) => {
  ...
  const props = omit(item, ['key', 'text', 'visible', 'query', 'tooltip', 'isMore', 'PopconfirmConfig', 'ModalConfirmConfig'])
  const defaultProps = {
    type: 'link',
    size: 'small',
    children: text
...
export default ({ dataIndex, transform, template }, context) => {
  ...
  return (text, record, index) => {
    ...
    // 拖拽手柄
    if (tpl === 'sort') {
      const { handler = <MenuOutlined />, disabledSort = null } = template
    ...
    if (tpl === 'link') {
      ...
      return [
        ...
        !isEmptyArray(dropdownList) && (
          <Dropdown key="Dropdown" overlayClassName={getClassNames('render-link-dropdown')} menu={{ items }} placement="bottomRight" arrow>
    ...
    if (tpl === 'avatar') {
      ...
      if (value === emptyText || isEmptyArray(value)) {
        return <Avatar icon={<UserOutlined />} {...omit(props, groupProps)} />
      }
      if (Array.isArray(value)) {
        return (
          <Avatar.Group {...props}>
