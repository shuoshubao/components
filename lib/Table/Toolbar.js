...
import { Tooltip, Popover, Checkbox, Button } from 'antd'
...
class Index extends Component {
  ...
  render() {
    ...
    const { showFullScreen, showColumnsSetting, showViewMode, getPopupContainer } = props
    const { isTable, fullscreened, viewModeStorageKey } = state
    const FullscreenIcon = fullscreened ? FullscreenExitOutlined.default : FullscreenOutlined.default
    const ViewSwitcherIcon = isTable ? AppstoreOutlined.default : MenuOutlined.default
    return (
      <>
        {!!showFullScreen && (
          <Tooltip
        ...
        {!!showColumnsSetting && (
          ...
          >
            <Tooltip
              ...
              getPopupContainer={getPopupContainer}
            >
              <SettingOutlined.default className={getClassNames('toolbar-icon')} />
            </Tooltip>
          </Popover>
        )}
        {!!showViewMode && (
          <Tooltip
            title={['切换至', isTable ? '列表' : '表格', '视图'].join('')}
            placement="top"
            arrowPointAtCenter
            getPopupContainer={getPopupContainer}
          >
