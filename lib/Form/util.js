...
import { classNames, formatTime, getTooltipHtml, isEmptyArray, isEmptyValue, isEveryFalsy, isSomeFalsy, plus } from '@nbfe/tools'
import { cloneDeep, find, get, isFunction, merge, omit } from 'lodash'
import moment from 'moment'
import { componentName, defaultColumn, inputTypeList, pickerFormatMap, prefixClassName } from './config'
...
export const mergeColumns = (columns = [], props = {}) => {
  ...
  return cloneDeep(columns).map(v => {
    ...
    if (tpl === 'tabs') {
      ...
      column.template.size = column.template.size || 'small'
    }
    if (tpl === 'cascader') {
...
export const getInitialValues = (columns = [], initialValues = {}) => {
  return cloneDeep(columns).reduce((prev, cur) => {
    ...
    const initialValue = initialValues?.[name] ?? cur.initialValue
    const { tpl } = template
    // 日期范围, 时间范围, 数字范围
    if (['date-range-picker', 'time-range-picker', 'number-range'].includes(tpl)) {
      const [key1, key2] = name.split(',')
