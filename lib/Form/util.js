...
import { classNames, isSomeFalsy, isEveryFalsy, formatTime, isEmptyValue, isEmptyArray, getTooltipHtml } from '@nbfe/tools'
import { componentName, prefixClassName, defaultColumn, pickerFormatMap, searchSeparator, inputTypeList } from './config'
...
export const getFormItemLabelWidth = columns => {
  ...
  const labelWidthList = columns.map(v => {
    const { required, label, tooltip, colon, rules } = v
    // 是否有星号
    const isRequired =
      required !== undefined
        ? required
    ...
    let labelWidth = label.length * 14
    if (isRequired) {
      labelWidth += requiredMarkWidth
    }
    if (tooltip) {
      labelWidth += tooltipWidth
    }
    if (colon) {
      labelWidth += colonWidth
    }
