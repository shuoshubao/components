...
import { Form, Card, Button, Space, Tooltip, Typography, message } from 'antd'
import BuiltInComponents from './components'
import { PlusOutlined, DeleteOutlined } from '@ant-design/icons'
import { componentName, defaultExtraConfig } from './config'
...
class Index extends Component {
  ...
  async componentDidMount() {
    ...
    const innerColumns = mergeColumns(columns, this.props.initialValues)
  ...
  renderColumns = () => {
    ...
    const { extraConfig, children } = props
    const { columns } = state
    ...
    const columnsNode = columns
      ...
      .map((v, i) => {
        ...
        if (formListConfig) {
          ...
          const emptyLabelNode = <Text style={{ width: labelWidth }} />
          return (
            <Form.List
              name={name}
                    ...
                    {fields.map((field, index) => {
                      return (
                        <Form.Item
                          ...
                          colon={index === 0}
                        >
                          <Form.Item {...field} rules={formItemProps.rules} validateTrigger={['onChange', 'onBlur']} noStyle>
                            {formItemNode}
                          </Form.Item>
  ...
  renderSearchReset = () => {
    ...
    return (
      <Form.Item
        colon={false}
        label={submitLabelWidth === 0 ? null : <div style={{ width: isNull(submitLabelWidth) ? labelWidth : submitLabelWidth }} />}
      ...
      >
        <Space>
          ...
          {showReset && <Button onClick={onReset}>{resetText}</Button>}
        </Space>
      </Form.Item>
    )
  }
  ...
  render() {
    ...
    const formProps = omit(props, ['columns', 'autoSubmit', 'showSearchBtn', 'showResetBtn', 'labelWidth', 'visibleFilterPanel', 'extraConfig'])

    formProps.onFinish = this.onSearch
